...
def load_exomol_transition(filename: str, chunksize: bool = None) -> pd.DataFrame:
    ...
    return pd.read_csv(
        ...
        usecols=[0, 1, 2],
        names=["upper", "lower", "Afi"],
        chunksize=chunksize,
    )


def combine_states_transitions(
    states: ExomolStates, transitions: pd.DataFrame
) -> t.Union[u.Quantity, u.Quantity, u.Quantity, np.ndarray]:
    new_df = transitions.merge(
        states.dataframe, how="left", left_on="upper", right_on="ID"
    ).merge(
        states.dataframe,
        how="left",
        left_on="lower",
